...
* About
** Value proposition
Every project that the Irish Local enterprise office (LEO) works with needs to be assigned their corresponding NACE/CPA code. As of 2023 NACE/CPA doesn't have a good interface and LEO employees are spending many hours on the lookup that they have to do manually.
...
* Tasks
** TODO Make sure we have the right versions of each specification
- Possibly change file names to reflect that.

** TODO Restart Git history from 0

** TODO Copy all fields as they are, no replacements
- CPA includes/also includes etc, preserve fields as they are, even copy over excludes.
...
** TODO Prepare L4 mapping example
